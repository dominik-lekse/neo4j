...
case object Daterange extends Function {
  def name = "daterange"

  def semanticCheck(ctx: ast.Expression.SemanticContext, invocation: ast.FunctionInvocation) : SemanticCheck =
    checkMinArgs(invocation, 2) then checkMaxArgs(invocation, 3) then
      when(invocation.arguments.length >= 2) {
        invocation.arguments(0).constrainType(StringType(), DateType()) then
        invocation.arguments(1).constrainType(StringType(), DateType())
  ...
  def toCommand(invocation: ast.FunctionInvocation) = {
    ...
    commandexpressions.DaterangeFunction(commands(0), commands(1), commands.lift(2))
  }
}